...
function ErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="text-center py-16">
      ...
      <p className="text-sm text-gray-500 mb-4">Couldn&apos;t load experience data.</p>
      <button type="button" onClick={onRetry} className="text-sm font-medium text-gray-700 hover:text-gray-900 underline underline-offset-2 transition-colors">
        Try again
      </button>
    </div>
  );
...
function EmptyState() {
  return (
    <div className="text-center py-16">
      ...
      <p className="text-sm text-gray-500">No experience listed yet. Check back soon.</p>
    </div>
  );
}
...
function TimelineItem({ item }: { item: Experience }) {
  ...
  return (
    <div ref={ref} className={`relative pl-8 pb-12 last:pb-0 border-l-2 border-gray-200 fade-in ${isInView ? "visible" : ""}`}>
      <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-white border-2 border-gray-400" />
      <div>
        <span className="inline-block text-xs font-medium text-gray-400 uppercase tracking-wider">{item.period}</span>
        <h3 className="mt-1.5 text-lg font-semibold text-gray-900">{item.role}</h3>
        <p className="text-sm text-gray-500 mt-0.5">{item.company}</p>
        <ul className="mt-3 space-y-1.5">
          {item.description.map((bullet) => (
            <li key={bullet} className="text-sm text-gray-600 leading-relaxed flex items-start gap-2">
              <span className="block w-1 h-1 rounded-full bg-gray-400 mt-2 shrink-0" />
              {bullet}
            </li>
          ))}
        </ul>
        <p className="mt-3 text-xs text-gray-400 font-medium">{item.stack}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-white pt-24">
      ...
      <section ref={ref} className="px-6 pb-24">
        <div className="max-w-3xl mx-auto">
          <h1 className={`text-2xl sm:text-3xl font-bold tracking-tight text-gray-900 fade-in ${isInView ? "visible" : ""}`}>
            Experience
          </h1>
          <p className={`mt-2 text-gray-500 fade-in stagger-2 ${isInView ? "visible" : ""}`}>
